...
# an extension for contrib/download-frozen-image-v2.sh of moby project
# it will use client certificate for authentication in private docker registry
...
usage() {
	...
	echo "  DOCKER_REGISTRY: location of the registry. Default localhost"
	echo "  DOCKER_IMAGES_DST: base location for images. Default /images"
	echo "  DOCKER_CLIENT_CERT: location of client certificates. Default /certs"
	[ -z "$1" ] || exit "$1"
}
...
imagesBase="${DOCKER_IMAGES_DST:-/images}"
certBase="${DOCKER_CLIENT_CERT:-/certs}"

originalDir=$1
dir="$imagesBase/$originalDir" # dir for building tar in
shift || usage 1 >&2

[ $# -gt 0 -a "$dir" ] || usage 2 >&2
mkdir -p "$dir"

registryBase="https://$DOCKER_REGISTRY"
...
fetch_blob() {
	...
	local curlHeaders="$(
		curl -S "${curlArgs[@]}" \
			--cacert "$certBase/ca.crt" --key "$certBase/client.key" --cert "$certBase/client.cert" \
...
handle_single_manifest_v2() {
	...
	fetch_blob "$image" "$configDigest" "$dir/$configFile" -s
	...
	for i in "${!layers[@]}"; do
		...
		case "$layerMediaType" in
			application/vnd.docker.image.rootfs.diff.tar.gzip)
				...
				fetch_blob "$image" "$layerDigest" "$dir/$layerTar" --progress-bar
				;;
	...
	case "$schemaVersion" in
		...
		1)
			...
			for i in "${!layers[@]}"; do
				...
				fetch_blob "$image" "$imageLayer" "$dir/$layerId/layer.tar" --progress-bar
			done
			;;
...
echo "Archiving into '${dir}.tar.gz'"
cd $dir 
tar -czf ${dir}.tar.gz *
cd -
rm -rf $dir

echo "To load the pulled image use:"
echo "  docker load --input images/${originalDir}.tar.gz"
echo "For Singularity, convert the docker to sif format"
echo "  singularity build images/${originalDir}.sif docker-archive://images/${originalDir}.tar.gz"